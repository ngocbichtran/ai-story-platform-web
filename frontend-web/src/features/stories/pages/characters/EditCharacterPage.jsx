...
export default function EditCharacterPage() {
    ...
    // 1. Lấy danh sách nhân vật để liên kết
    useEffect(() => {
        const fetchAllCharactersInStory = async () => {
            try {
                ...
                if (res.data.success) {
                    const list = (res.data.data || []).filter((c) => String(c.id) !== String(characterId));
        ...
        const fetchCharacterDetail = async () => {
            ...
            try {
                ...
                if (res.data.success) {
                    ...
                    setFormData({
                        name: char.name || "",
                        role: char.role || "Nhân vật chính",
                        gender: char.gender || "Nam",
                        age: char.age !== undefined && char.age !== null ? char.age : "",
                        occupation: char.occupation || "",
                        appearanceFinal: char.appearance || "",
                        ...
                        relationship: Array.isArray(char.relationship) ? char.relationship : [],
                    });
                }
            } catch (err) {
    ...
    const handleEditRelationshipClick = (item) => {
        const targetRelId = item.characterId || item.id;
        setEditingRelationId(targetRelId);
        setModalData({
            selectedCharId: String(targetRelId),
            relationType: item.relationType || "Đồng minh",
    ...
    const handleSaveRelationship = (e) => {
        ...
        const targetChar = allCharacters.find((c) => String(c.id) === String(modalData.selectedCharId));
        if (!targetChar) {
            toast.error("Không tìm thấy nhân vật được chọn.");
            return;
        }
        const relationPayload = {
            characterId: targetChar.id,
            name: targetChar.name,
            relationType: modalData.relationType.trim() || "Đồng minh",
            description: modalData.description.trim(),
        };
        if (editingRelationId !== null) {
            setFormData((prev) => ({
                ...prev,
                relationship: prev.relationship.map((r) => (String(r.characterId || r.id) === String(editingRelationId) ? relationPayload : r)),
            }));
        } else {
            const exists = formData.relationship.some((r) => String(r.characterId || r.id) === String(targetChar.id));
            if (exists) {
    ...
    const handleDeleteRelationship = (id) => {
        setFormData((prev) => ({
            ...prev,
            relationship: prev.relationship.filter((item) => String(item.characterId || item.id) !== String(id)),
        }));
    };
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            console.error("Lỗi cập nhật nhân vật:", err);
            const errorMsg = err.response?.data?.message || "Không thể cập nhật thông tin nhân vật.";
                    ...
                    <div className="grid grid-cols-12 items-center gap-3 p-2 relative z-10 w-full">
                        ...
                        <div className="col-span-12 md:col-span-4 flex items-center gap-3 rounded-xl border border-white/5 bg-slate-950/30 p-2">
                            ...
                            <div className="min-w-0 flex-1">
                                <h1 className="truncate text-lg font-black">{formData.name || "Chỉnh sửa nhân vật"}</h1>
                            </div>
                        </div>
                                        ...
                                        <div className="space-y-3.5 text-sm">
                                            ...
                                            <div className="flex flex-col gap-1.5">
                                                ...
                                                <div className="relative">
                                                    ...
                                                    <input type="text" name="currentLocation" value={formData.currentLocation} onChange={handleChange} placeholder="Vị trí hiện tại..." className="w-full bg-slate-950/50 border border-white/10 rounded-xl pl-9 pr-4 py-2 text-sm text-white focus:outline-none focus:border-blue-500/50 transition placeholder-slate-600" />
                                                </div>
                                            </div>
                                            <div className="flex flex-col gap-1.5">
                                                <label className="text-xs font-semibold uppercase tracking-wider text-slate-400">URL Ảnh đại diện</label>
                                                ...
                                                {formData.tags.length > 0 && (
                                                    <div className="flex flex-wrap gap-1.5 mt-2">
                                                        {formData.tags.map((tag, idx) => (
                                                            <span key={`${tag}-${idx}`} className="flex items-center gap-1 px-2.5 py-1 rounded-lg bg-blue-500/15 border border-blue-500/30 text-xs text-blue-300">
                                                                <Tag size={11} />
                                                                {tag}
                                    ...
                                    <section className="lg:col-span-4 rounded-2xl border border-white/10 bg-slate-900/30 backdrop-blur-xl shadow-xl flex flex-col h-[520px] overflow-hidden">
                                        <div className="mb-2 flex items-center justify-between border-b border-white/10 p-5 pb-3 shrink-0">
                                            ...
                                            <button
                                                ...
                                                className="flex items-center gap-2 rounded-xl bg-gradient-to-r from-blue-600 to-indigo-600 px-4 py-2 text-xs font-semibold text-white transition hover:opacity-90 active:scale-95 shadow-md shadow-blue-900/20"
                                            >
                                                <Plus size={16} />
                                                Thêm
                                            </button>
                                            ...
                                            {formData.relationship.length === 0 ? (
                                                ...
                                                <div className="flex flex-col gap-2.5 w-full">
                                                    {formData.relationship.map((item, index) => {
                                                        ...
                                                        return (
                                                            <div key={`${relId}-${index}`} className="group flex items-center justify-between rounded-2xl border border-white/5 bg-white/[0.02] px-4 py-3 transition-all duration-300 hover:border-blue-500/30 hover:bg-blue-500/[0.04]">
                                                                <div className="flex items-center gap-3 min-w-0">
                                                                    <div className="min-w-0">
                                                                        <h3 className="text-sm font-semibold text-white truncate">{item.name || "Nhân vật liên kết"}</h3>
                                                                        <p className="mt-0.5 text-xs text-slate-400 truncate">{item.relationType || "Chưa xác định"}</p>
            ...
            {isModalOpen && (
                <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm">
                    <div className="relative w-full max-w-md rounded-2xl border border-white/10 bg-[#0B1120] p-6 shadow-2xl flex flex-col gap-5">
                        <button type="button" onClick={resetRelationshipModal} className="absolute top-4 right-4 text-slate-400 hover:text-white transition">
                            <X size={18} />
                        </button>
                        <div className="flex items-center gap-3 border-b border-white/10 pb-3">
                            <Users size={20} className="text-blue-400" />
                            <h3 className="text-lg font-bold text-white">{editingRelationId !== null ? "Cập nhật mối quan hệ" : "Thêm mối quan hệ"}</h3>
                        </div>
                        ...
                        <div className="flex gap-3 justify-end border-t border-white/10 pt-4 mt-2">
                            <button type="button" onClick={resetRelationshipModal} className="rounded-xl border border-white/10 bg-white/5 px-4 py-2 text-xs font-semibold text-slate-300 hover:bg-white/10 transition">
                                Hủy bỏ
                            </button>
                            <button type="button" onClick={handleSaveRelationship} className="rounded-xl bg-gradient-to-r from-blue-600 to-indigo-600 px-5 py-2 text-xs font-bold text-white transition hover:opacity-90 active:scale-95">
                                {editingRelationId !== null ? "Lưu thay đổi" : "Xác nhận thêm"}
                            </button>
                        </div>
                    </div>
                </div>
