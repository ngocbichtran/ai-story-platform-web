...
export default function EditStory() {
    ...
    useEffect(() => {
        const fetchInitialData = async () => {
            try {
                ...
                console.error("Lỗi tải thông tin chỉnh sửa:", err);
                toast.error("Không thể tải thông tin tác phẩm.");
                navigate("/stories");
            } finally {
                setIsLoading(false);
            }
    ...
    const handleSave = async () => {
        ...
        if (selectedGenres.length === 0) return toast.error("Vui lòng chọn thể loại!");
        ...
        try {
            ...
            console.error("Lỗi cập nhật tác phẩm:", err);
            toast.error(err.response?.data?.message || "Có lỗi xảy ra khi cập nhật truyện.");
        } finally {
            setIsSaving(false);
        }
    };
    ...
    if (isLoading) {
        return (
            <div className="min-h-screen bg-[#0B1120] flex items-center justify-center text-white">
                ...
                <span>Đang đồng bộ dữ liệu tác phẩm...</span>
            </div>
        );
    }
                            ...
                            <div className="space-y-6">
                                ...
                                <div className="flex items-center gap-4">
                                    ...
                                    <button type="button" onClick={() => setShowGenreModal(true)} className="flex-1 min-h-12 rounded-xl border border-white/10 bg-white/5 px-4 py-3 text-left text-slate-300 hover:bg-white/10 transition">
                                        {selectedGenres.length ? selectedGenres.map((g) => g.name || g).join(", ") : "Chọn thể loại"}
                                    </button>
                                </div>
                    ...
                    <div className="w-[440px] rounded-2xl border border-white/10 bg-[#111827] p-6 shadow-2xl">
                        ...
                        <div className="mb-4">
                            <div className="flex gap-2">
                                ...
                                <button onClick={handleCreateGenre} className="rounded-xl bg-violet-600 px-4 hover:bg-violet-700 font-semibold text-sm">
                                    Thêm
                                </button>
                            </div>
                        </div>
                        ...
                        <div className="mt-6 flex justify-end gap-2 border-t border-white/5 pt-4">
                            ...
                            <button onClick={() => setShowGenreModal(false)} className="rounded-xl bg-gradient-to-r from-blue-600 to-violet-600 px-4 py-2 text-sm font-semibold">
                                Xác nhận
                            </button>
                        </div>
                    </div>
